"""
Calcule la moyenne climatologique pour chaque mois des variables 3D d'océano lu sur un serveur à distance lors de l'execution du code. Renvoi un fichier netcdf contenant les moyennes climatiques pour chaque mois de l'année.
Le code est long à tourner car il lit au fur et à mesure les données sur le serveur à distance.
"""


from netCDF4 import Dataset
import os
import xarray as xr
import warnings
import matplotlib.pyplot as plt
warnings.filterwarnings("ignore")
import numpy as np
from scipy.interpolate import interp2d
import cartopy.crs as ccrs

#print(ds.info())

###OPEN FILE
username = "vbremaud"
password = "Vincent1"
URL = "my.cmems-du.eu/thredds/dodsC/global-reanalysis-phy-001-031-grepv2-monthly" #sans le http://
OUTPUT = "Climatology_oceano_3d.nc"

### Input

Indice_MOIS = np.arange(0,12,1) #numéro du mois
Variable_obs = ["uo_oras", "vo_oras","thetao_oras"]

LATITUDE = slice(-30,30) #opposé à ERA5
LONGITUDE = 1439 #dernière longitude de la liste

#uo_oras, vo_oras, thetao_oras,

MOIS = ['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Spetembre','Octobre','Novembre','Décembre']

### Traitement

X_DS = (
    xr.open_dataset("https://"+username+":"+password+"@"+URL))
print(X_DS)

VAR_DATA = []
VAR_UNITS = []


VAR_DATA = []
VAR_UNITS = []

for i in range(len(Variable_obs)):
    VAR_DATA_TIME=[]
    print(i)
    for t in Indice_MOIS :
        print(t)
        X = X_DS[Variable_obs[i]].sel(latitude=LATITUDE).isel(longitude=LONGITUDE)
        print(X.shape)
        I = np.arange(t,len(X[:,0,0]),12)  #modulo 12 car 12 mois dans l'année

        unit = X.units
        lats = X.coords['latitude']
        lons = X.coords['longitude']
        depths = X.coords['depth']
        X_mois =[X.data[i,:,:] for i in I]

        X_mean = np.nanmean(X_mois,axis=0)
        VAR_DATA_TIME.append(X_mean)
    VAR_UNITS.append(unit)
    VAR_DATA.append(VAR_DATA_TIME)


ds = Dataset(OUTPUT, mode="w")

ds.set_fill_off()

time = ds.createDimension('time', len(MOIS))
#longitude = ds.createDimension('longitude', len(lons))
latitude = ds.createDimension('latitude', len(lats))
depth = ds.createDimension('depth', len(depths))

times = ds.createVariable('time', 'f4', ('time',))
#lons1 = ds.createVariable('longitude', 'f4', ('longitude',))
lats1 = ds.createVariable('latitude', 'f4', ('latitude',))
depths1 = ds.createVariable('depth', 'f4', ('depth',))

uo = ds.createVariable('uo', 'f4', ('time', 'depth','latitude'))
vo = ds.createVariable('vo', 'f4', ('time', 'depth','latitude'))
thetao = ds.createVariable('thetao', 'f4', ('time', 'depth','latitude'))

times[:]=np.array(Indice_MOIS)+1
lats1[:]=lats
#lons1[:]=lons
depths1[:]=depths

#add netcdf attributes
uo.units = VAR_UNITS[0]
vo.units = VAR_UNITS[1]
thetao.units = VAR_UNITS[2]

for i in range(len(Indice_MOIS)):
    uo[i,:,:]=np.array(VAR_DATA[0][i])
    vo[i,:,:]=np.array(VAR_DATA[1][i])
    thetao[i,:,:]=np.array(VAR_DATA[2][i])


ds.close();