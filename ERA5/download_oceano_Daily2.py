"""
Récupère des données journalières d'océano.
"""
...
username = "vbremaud"
password = "Vincent1"
URL = "my.cmems-du.eu/thredds/dodsC/global-reanalysis-phy-001-031-grepv2-daily" #sans le http://
OUTPUT = "oceano_daily.nc"
...
for i in range(len(ANNEE)):
    ...
    Xdata = X.data
    VAR_TIME.append(list(np.arange(1+i*365,366+i*365,1)))
...
time = ds.createDimension('time', 365*len(ANNEE))
...
for i in range(len(ANNEE)):
    times1[i*365:(i+1)*365]=np.array(VAR_TIME[i])
    zos[i*365:(i+1)*365,:,:] = np.array(VAR_DATA[i])